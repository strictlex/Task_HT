...
def pos_points(file1, file2):
    with open(
        ...
        x, y = [
            int(i) for i in file1.readline().rstrip().split()
        ]  # координаты центра элипса
    ...
    with open(
        ...
        points = [
            ...
            for i in [el.strip() for el in file2.readlines()]
        ]  # список отчек
        positions = []
    for i in points:
        answer = (
            ((i[0] - x) ** 2 / r1**2) + ((i[1] - y) ** 2 / r2**2) - 1
        )  # формула положения точки относительно элипса
        if answer == 0:
